...
const Reserve = () => {
    const [startDate, setStartDate] = useState(setHours(setMinutes(new Date(), 0), 14));
    ...
    useEffect(() => {
        ...
        const uuid = startDate.getFullYear().toString() + ("0" + (startDate.getMonth() + 1)).slice(-2).toString() +
            ("0" + startDate.getDate()).slice(-2).toString() + startDate.getHours().toString() + "00";
        const check = async () => {
            try {
                const turno = await axios.post('/api/reserva/check', { turnoId: uuid, personas: personas });
    ...
    const personasChange = async (e) => {
        ...
        setPersonas(p);
        try {
            const turno = await axios.post('/api/reserva/check', { turnoId: turnoId, personas: p });
            console.log(turno.data.available)
    ...
    return (
        <>
            ...
            {disponible ? <ReservaForm personas={personas} turnoId={turnoId} />
                : null}
        </>
    );
}
